...
radar_radius: 1.1                      # 可选，雷达本体图标圆点大小，默认 1.1
...
* **雷达状态与色彩标识**：
  * 🌟 **专属雷达在线**：显示为**暖阳琥珀金**，自带科技动态呼吸光环与环境微晕，名称及下拉列表自动冠以 `⭐` 星标徽标，尊贵醒目。
  * 🔵 **普通雷达在线**：显示为**科技蓝**。
  * 🔴 **掉线告警**：雷达掉线或实体不可用时自动高亮为**警告红**，并附带 `(Off)` 标注。
  * ⚫ **手动暂停**：显示为**低调深灰**并标注 `(⏸)`。
  * 🟡 **当前选中**：中心控制点高亮为**耀眼金色**，专属雷达的探测扇区与角度控制线同步联动为琥珀色。
...
* `⏸` 暂停雷达：点击可单独暂停该雷达的数据处理与点云刷新，方便调试。

* **⭐ 专属硬件功能（需搭配 RMM 专属雷达硬件，即将发布）**：
  * 🌐 **一键直达雷达 Web 控制台**：系统自动探测雷达的局域网 IP，点击按钮即可一键在新标签页打开雷达管理后台。
  * ☁️ / ⬆️ **快捷检查与固件升级 (Cloud OTA)**：自动感知并展示雷达固件版本，当有新固件时会自动高亮橙色与小红点提醒，支持在地图卡片中一键发起 OTA 升级。
